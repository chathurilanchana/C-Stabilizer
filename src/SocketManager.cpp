...
int SocketManager::SetLimits(unsigned int _uiSoftLimit, unsigned int _uiHardLimit)
{
	rlimit limits;
...
unsigned int SocketManager::FireAll()
{
	...
	for (unsigned int ui = ui_CurrentPos; ui < (ui_CurrentPos + ui_TotalDeviation); ++ui)
	{
		++uiTemp;
...
Timer * SocketManager::CreateTimer(double _dIntaval, bool _bIsRepeated, TimerCallback * _pTCB, int _iTimerType)
{
	unsigned int uiTimerID = GetTimerIDFromStack();
	unsigned int uiInt = (unsigned int) (_dIntaval * 1000);

	Timer * pTimer = new Timer(uiInt, _pTCB, _bIsRepeated, uiTimerID, this, _iTimerType);
	m_ptrTimers[uiTimerID] = pTimer;
	AddToTimerList(pTimer);
	return pTimer;
}

Client * SocketManager::CreateClient(const char * _zServerIP, unsigned int _uiServerPort, ClientCallback * _pCCB, bool _bHB)
{
	...
	{
		cout << "[ERROR ]epoll_ctl creation failed .  IP " << _zServerIP << " Port " << _uiServerPort << endl;
		return NULL;
	}
	Client * pClient = new Client(_zServerIP, _uiServerPort, iSocket, _pCCB, this, NULL);
	m_ptrServerTypes[iSocket] = pClient;
	return pClient;

}

Server * SocketManager::CreateServer(unsigned int _uiPort, ServerCallback * _pSCB, bool _bHB)
{
	int iSocket = -1;
...
void SocketManager::AddToDeletedClientSet(unsigned int _uiClientID)
{
	...
	{
		cout << "[ERROR] Removing a Client from epoll failed.  Client = " << _uiClientID << endl;
	}
	close(_uiClientID);
}
void SocketManager::AddToDeletedServerSet(unsigned int _uiServerID)
{
	...
	{
		cout << "[ERROR] Removing a server from epoll failed.  Server = " << _uiServerID << endl;
	}
	close(_uiServerID);
}
			...
			if (pEItem)
			{
				if (pEItem->getEvtType() == E_EVT_SERVER)
				{
					...
					while (true)
					{
						...
						{
							cout << " [ERROR] Adding a client to epoll failed. Client = " << iCliSock << " Server = " << iReadySock << endl;
							continue;
						}
						...
						{
							pClient = new Client("UnknownIP", 0, (unsigned int) iCliSock, NULL, this, pTemServer);
						...
						else
						{
							...
							iPort = addr.sin_port;
							pClient = new Client(zCliIp, (unsigned int)iPort, (unsigned int) iCliSock, NULL, this, pTemServer);
							m_ptrServerTypes[iCliSock] = pClient;
							ServerCallback * pSCB = pTemServer->GetServerCB();
							pSCB->OnConnect(pTemServer, pClient);
							pTemServer->AddtoConnectedClientsList(pClient);
				...
				else
				{
					...
					if (pstEpollRetEvents[i].events & EPOLLOUT) //can write to this client
					{
						...
						if (iBufLen == iSendLen)
						{
							...
							{
								cout << "[ERROR] Modifying epoll failed. Client = " << iCliId << endl;
							}
	...
	if (set_DiconClients.size() > 0)
	{
		...
		while (ite != set_DiconClients.end())
		{
			...
			else
			{
				...
				{
					cout << "[ERROR] Removing a client from epoll failed.  Client = " << iClientId << endl;
				}
			}
			++ite;
		}
		...
		while (iSendLen < iTotLen)
		{
			if ((iCurSend = write(iClientId, zBuf + iSendLen, iBufLen)) < 0)
			{
				if (errno == EAGAIN && iBufLen > 0)
				{
					...
					{
						cout << "[ERROR] Modifying epoll failed. Client = " << iClientId << endl;
					}
					return iSendLen;
				}
				else
...
void SocketManager::Detach(Client * _pClient)
{
	...
	{
		cout << "[ERROR] Removing a Client from epoll failed.  Client = " << iClid << " Error:[" << errno << "] " << strerror(errno) << endl;
	}
...
void SocketManager::AttachClient(Client * _pClient, ClientCallback * _pCCB)
{
	...
	{
		cout << "[ERROR] epoll_ctl failed. CLID " << iClid << " Error:[" << errno << "] " << strerror(errno) << endl;
	}
}
